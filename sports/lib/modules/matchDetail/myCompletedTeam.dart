...
class MyCompleteTeam extends StatefulWidget {
  final match_id, type, match, time;
  List<GetTeam> data;
  Function onTap;

  MyCompleteTeam(
      this.match_id, this.type, this.match, this.time, this.data, this.onTap);

  @override
  State<MyCompleteTeam> createState() => _MyCompleteTeamState();
}

class _MyCompleteTeamState extends State<MyCompleteTeam> {
  // List<GetTeam> data;
  bool isLive = false;
      ...
      child: Container(
          child: widget.data != null && widget.data.length > 0
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => TeamPreViewPage(
                                      getTeamA(count),
                                      getTeamB(count)[0].pid,
                                      getTeamB(count)[1].pid,
              ...
              : Container(
                  ...
                  child: Center(child: Text('You have no teams saved yet')),
                )),
    );
  }
  ...
  getTotalPoint(count) {
    ...
    for (var i in widget.data[count].team_A.toString().split(',')) {
      for (var j in ConstanceData.teamCombine_points) {
        if (i == j.pid.toString()) {
          if (i == widget.data[count].team_B.toString().split(',')[0]) {
            totalpoint += (double.parse(j.point.toString()) * 2);
          } else if (i == widget.data[count].team_B.toString().split(',')[1]) {
