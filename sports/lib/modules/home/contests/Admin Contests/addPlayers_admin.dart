...
import 'chooseCaptain_admin.dart';

class AddPlayersAdmin extends StatefulWidget {
  final list, index, contestId, time, compId, matchID, coupon, data, teams;

  AddPlayersAdmin(this.list, this.index, this.contestId, this.time, this.compId,
      this.teams, this.matchID, this.coupon, this.data);

  @override
  State<AddPlayersAdmin> createState() => _AddPlayersAdminState();
}

enum selectedTeam { TeamA, TeamB, Both }

class _AddPlayersAdminState extends State<AddPlayersAdmin> {
  bool isWk = true;
  bool isBAT = false;
  bool isAR = false;
  bool isBowl = false;
                        ...
                        if (BOWL >= 3 && BOWL <= 6) {
                          if (ConstanceData.selectedPlayers.length == 11) {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => ChooseCaptain_Admin(
                                    ConstanceData.selectedPlayers,
                                    widget.compId,
                                    widget.matchID,
                                    widget.time,
